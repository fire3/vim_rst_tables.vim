...
def get_field_header_width(field_text):
    return max(map(lambda s: get_unicode_size(s), field_text.split('\n')))

def get_column_header_widths(table):
    widths = []
    for row in table:
        num_fields = len(row)
        # dynamically grow
        ...
        for i in range(num_fields):
            ...
            strr = unicode(field_text,"utf8")
            get_unicode_size(strr)
            #field_width = get_field_width(field_text)
            field_width = get_field_header_width(strr)
            widths[i] = max(widths[i], field_width)  + 1
    return widths
...
def get_column_widths(table):
    ...
    for row in table:
        ...
        for i in range(num_fields):
            ...
            strr = unicode(field_text,"utf8")
            #field_width = get_field_width(field_text)
            field_width = get_field_width(strr)
...
def pad_fields(row, widths):
    ...
    for i in range(len(row)):
        col = row[i]
        col_unicode = unicode(col,"utf8")
        #print "%d, %d"%(widths[i],get_unicode_size(col_unicode))
        #print ("%s%-"+str(eval(widths[i]) -  len(col_unicode))+"s")
        col = (" %s%-"+str((widths[i]) -  get_unicode_size(col_unicode))+"s ")%(col_unicode,' ')
        #col = widths[i] % col_unicode.strip()
        col = col.encode("utf8")
        #print "%i:%s"%(i,col)
...
def draw_table(table, manual_widths=None):
    ...
    if manual_widths is None:
        col_widths = get_column_header_widths(table)
    else:
